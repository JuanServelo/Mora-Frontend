        ...
        <div className="space-y-3">
          {recFiltradas.map((rec) => (
            <div key={rec.id} className="glass-panel rounded-2xl overflow-hidden">
              {/* Cabeçalho */}
              <button
                className="w-full flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 sm:px-6 py-4 text-left cursor-pointer hover:bg-veu/5 transition-all"
                onClick={() =>
              ...
              {/* Detalhes expandidos */}
              {expandido === rec.id && (
                <div className="px-4 sm:px-6 pb-5 border-t border-veu/5 pt-4 space-y-5">
                  {/* Info */}
